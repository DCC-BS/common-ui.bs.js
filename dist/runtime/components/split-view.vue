...
const props = defineProps({
  aPaneStyle: { type: String, required: false, default: "" },
  bPaneStyle: { type: String, required: false, default: "" },
  splitViewStyle: { type: String, required: false, default: "" },
  resizerStyle: { type: String, required: false, default: "" },
  resizerInnerStyle: { type: String, required: false, default: "" },
...
const verticalResizerStyle = "w-[16px] cursor-col-resize relative pointer-events-auto flex justify-center";
...
const horizontalResizerStyle = "h-[16px] cursor-row-resize relative pointer-events-auto flex items-center";
const horizontalResizerInnerStyle = "bg-gray-100 h-[2px] w-full";
const aPaneCssStyle = ref();
const bPaneCssStyle = ref();
...
const resize = (e) => {
  ...
  const container = e.target.closest(".split-view");
  if (props.isHorizontal) {
    const newHeight = (e.clientY - container.offsetTop) / container.offsetHeight * 100;
    const h = Math.min(Math.max(newHeight, 1), 99);
    aPaneCssStyle.value = { height: `${h}%` };
    bPaneCssStyle.value = { height: `${100 - h}%` };
  } else {
    const newWidth = (e.clientX - container.offsetLeft) / container.offsetWidth * 100;
