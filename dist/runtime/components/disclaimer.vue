...
const disclaimerAcceptedVersion = useLocalStorage(
  "disclaimerAccepted",
  ""
);
...
onMounted(() => {
  if (!confirmationText.value) {
    confirmationText.value = `Ich habe die Hinweise gelesen und verstanden und best\xE4tige, dass ich ${props.appName} ausschliesslich unter Einhaltung der genannten Richtlinien verwende.`;
  }
  isReady.value = true;
});
onUnmounted(() => {
...
<style scoped>
.disclaimer-modal{backdrop-filter:blur(8px);-webkit-backdrop-filter:blur(8px);background-color:hsla(0,0%,100%,.5);bottom:0;left:0;padding:1rem;position:fixed;right:0;top:0;z-index:99999999}.modal-container{background-color:#fff;border:1px solid #eee;box-shadow:0 2px 8px rgba(0,0,0,.1);height:100%;margin:auto;max-width:900px;overflow-y:auto;padding:2rem;position:relative}.scroll-down-button{align-items:center;background-color:#007acc;border:none;border-radius:50%;bottom:1rem;box-shadow:0 2px 12px rgba(0,122,204,.3);color:#fff;cursor:pointer;display:flex;height:48px;justify-content:center;position:fixed;right:1rem;transition:all .3s ease;width:48px;z-index:10}.scroll-down-button:hover{background-color:#005a9e;box-shadow:0 4px 16px rgba(0,122,204,.4);transform:scale(1.1)}.scroll-down-button:active{transform:scale(.95)}.scroll-down-button svg{animation:bounce 2s infinite}@keyframes bounce{0%,20%,50%,80%,to{transform:translateY(0)}40%{transform:translateY(-3px)}60%{transform:translateY(-2px)}}
</style>
